 ...
 * 内存handler的安全实现
 * <p>
 * 原始实现处理程序总是硬引用队列处理程序的执行。
 * 如果您创建匿名处理程序和延迟的消息后,它将把所有父类
 * 在该时间在内存中即使清洁。
 * <p>
 * 这个实现有一定难度,它将保持weakreference可运行和信息,
 * 和GC可以收集他们一旦WeakHandler实例不再引用
 */
...
public class WeakHandler {
    ...
    /**
    * 默认构造函数将这个处理程序与的{ @link弯}
     *当前线程。
     *如果线程没有looper ,这个处理程序将无法接收消息
     *所以抛出异常。
     */
    public WeakHandler() {
        mCallback = null;
     ...
     * 消息。
     * <p>
     如果这个线程没有looper,这个处理程序将无法接收消息
     *所以抛出异常。
     *
     * @param回调处理消息的回调接口,或null。
     */
     ...
     * 使用提供的{ @link  looper}而不是默认的一个。
     * @param looper The looper, must not be null. 不能为null
     */
     ...
     * 使用提供的{ @link looper}而不是默认的一个回调处理消息的接口。
    ...
    /**
     *导致Runnable r被添加到消息队列。
     *可将此处理程序的线程上运行
     *附呈。
     *
     * @param r 将执行的可运行。
     * @return返回true,如果运行的成功的
     *消息队列。回报;如果执行失败将返回false,这通常是因为
     *looper处理消息队列是退出。
     */
     ...
     *
     * *导致Runnable r被添加到消息队列,运行
     *在一个特定时间由< var > uptimeMillis < / var >。
     * < b >时基是{ @link android.os.SystemClock # uptimeMillis }。< / b >
     *可将此处理程序的线程上运行。
     *
     * @param r 将执行的可运行。
     * @param uptimeMillis 的绝对时间回调应该运行,
